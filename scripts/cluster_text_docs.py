...
# =============
USE_CACHE = True
CACHE_FOLDER_EBOOKS = os.path.expanduser('~/.cluster_ebooks')
CACHE_FOLDER_HTML = os.path.expanduser('~/.cluster_html')
...
def convert(input_file, output_file=None,
    ...
    if ocr_enabled == 'always':
        ...
        if ocr_file(input_file, output_file, mime_type, ocr_command, ocr_only_random_pages):
            logger.info("OCR failed! Will try conversion...")
        ...
        if statuscode == 0 and isalnum_in_file(output_file):
            ...
            logger.info("Conversion failed! Will try OCR...")
            if ocr_file(input_file, output_file, mime_type, ocr_command, ocr_only_random_pages):
                logger.warning(yellow("OCR failed!"))
                logger.warning(yellow(f"File couldn't be converted to txt: {input_file}"))
...
def setup_argparser():
    ...
    by_default_msg = 'By default, only HTML pages are clustered from within the specified directory.'
    msg = f'Cluster text documents (HTML pages or ebooks). {by_default_msg}'
    ...
    cache_group.add_argument(
        ...
        help='Use cache.')
    cache_group.add_argument(
        '--cfe', '--cache-folder-ebooks', metavar='PATH', dest='cache_folder_ebooks',
        default=CACHE_FOLDER_EBOOKS,
        help='Cache folder for ebooks.' + get_default_message(f'~/{Path(CACHE_FOLDER_EBOOKS).name}'))
    cache_group.add_argument(
        '--cfh', '--cache-folder-html', metavar='PATH', dest='cache_folder_html',
        default=CACHE_FOLDER_HTML,
        help='Cache folder for HTML pages.' + get_default_message(f'~/{Path(CACHE_FOLDER_HTML).name}'))
    cache_group.add_argument(
    ...
    dataset_group.add_argument(
        '-t', '--dataset-type', metavar='DATASET_TYPE', dest='dataset_type',
        default=DATASET_TYPE, choices=['html', 'ebooks'],
        help=f'Whether to cluster html pages or ebooks (pdf, djvu). {by_default_msg}'
             + get_default_message(DATASET_TYPE))
    dataset_group.add_argument(
...
class DatasetManager:
    ...
    def __init__(self, **kwargs):
        ...
        self.cache_folder_html = CACHE_FOLDER_HTML
        ...
        self.dataset_path = self.input_directory.joinpath(f'dataset_{self.dataset_type}.pkl')
        np.random.seed(self.seed)
        random.seed(self.seed)
        # Setup cache
        cache_size_bytes = round(self.cache_size_limit * float(1 << 30), 2)
        if self.use_cache:
            cache_folder = self.cache_folder_html if self.dataset_type == 'html' else self.cache_folder_ebooks
            self.cache = Cache(directory=cache_folder,
    ...
    def _generate_dataset(self):
        ...
        if self.dataset_type == 'ebooks':
            self._generate_ebooks_dataset()
        else:
            self._generate_html_dataset()
    ...
    def _generate_html_dataset(self):
        self.DESCR = "Dataset containing text from HTML pages"
        filepaths = [filepath for filepath in self.input_directory.rglob('*.html')]
        total = len(filepaths)
        add_text_to_cache = False
        text_added_dataset = []
        text_added_cache = []
        file_hashes = []
        duplicates = []
        for i, filepath in enumerate(self.input_directory.rglob('*.html'), start=1):
            logger.info(f"{COLORS['BLUE']}Processing document {i} of {total}:{COLORS['NC']} {filepath.name[:92]}")
            file_hash = None
            if self.update_dataset:
                if filepath in self.dataset_tmp.filenames:
                    idx = self.dataset_tmp.filenames.index(filepath)
                    logger.debug('Document already found in the loaded dataset')
                    text = self.dataset_tmp.data[idx]
                    self._add_doc_to_dataset(filepath, text)
                    continue
                else:
                    logger.info(yellow('Document not found in the loaded dataset. Will try to add it.'))
            if self.use_cache:
                file_hash = get_hash(filepath)
                logger.debug(f'File hash: {file_hash}')
                if file_hash in file_hashes:
                    ...
                    file_hashes.append(file_hash)
                text = self.cache.get(file_hash)
                if text:
                    logger.info('Found text in cache')
                else:
                    logger.debug('Text not found in cache. Will extract text from HTML page')
                    raw_text = read_file(filepath)
                    text = BeautifulSoup(raw_text, 'html.parser').get_text().replace('\xa0', ' ')
                    add_text_to_cache = True
            else:
                logger.debug('Extracting text from HTML page')
                raw_text = read_file(filepath)
                text = BeautifulSoup(raw_text, 'html.parser').get_text().replace('\xa0', ' ')
            logger.info('Adding text to dataset')
            self._add_doc_to_dataset(filepath, text)
...
def cluster_text_docs(data_manager):
    ...
    logger.info("")
    logger.info("# ========================================")
    logger.info("# Feature Extraction using TfidfVectorizer")
    logger.info("# ========================================")
    ...
    logger.info(f"Sparsity: {X_tfidf.nnz / np.prod(X_tfidf.shape):.3f}")
    ...
    logger.info("")
    logger.info("# ========================================")
    logger.info("# Clustering sparse data with random model")
    logger.info("# ========================================")
    ...
    logger.info("")
    logger.info("# ===================================")
    logger.info("# Clustering sparse data with k-means")
    logger.info("# ===================================")
    kmeans = KMeans(
        n_clusters=true_k,
        max_iter=100,
    ...
    logger.info("")
    logger.info("# KMeans with LSA on tf-idf vectors")
    logger.info("# ===============================")
    kmeans = KMeans(
        n_clusters=true_k,
        max_iter=100,
    ...
    logger.info("")
    logger.info("# MiniBatchKMeans with LSA on tf-idf vectors")
    logger.info("# ========================================")
    ...
    logger.info("")
    logger.info("# KMeans with LSA on hashed vectors")
    logger.info("# =================================")
    fit_and_evaluate(kmeans, X_hashed_lsa, name="KMeans\nwith LSA on hashed vectors")

    logger.info("")
    logger.info("# MiniBatchKMeans with LSA on hashed vectors")
    logger.info("# ==========================================")
    fit_and_evaluate(
        minibatch_kmeans,
        X_hashed_lsa,
...
def main():
    ...
    try:
        ...
        if args.use_cache or args.check_number_items or args.clear_cache or args.remove_keys:
            ...
            use_cache = False
        import_modules(dataset_type=args.dataset_type, english_detector='pycld2', use_cache=use_cache)
        # Get correct cache folder based on options
        if args.cache_folder_ebooks != CACHE_FOLDER_EBOOKS:
